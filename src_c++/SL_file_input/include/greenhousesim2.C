...
{   
	...
	alpha[0]= convective_coef_forced(4,geo.wind_speed);//0.95 + 6.76*pow(geo.wind_speed,0.49);  
	alpha[2] =  convective_coef_forced((geo.ext_land/2),geo.wind_speed);
...
void greenhouse::solver(int timelenght, int iteration_for_timestep){
	...
	for ( time=0; time<=timelenght; time++)
	{
		for (int i=0; i<=4000; i++) 
		{
			...
			alpha[4]= convective_coef_natural_2(6,T_air_inside,T[4]);
			...
			alpha[5]=1.246*pow(fabs(T[5]-T_air_inside),0.33);
                        //alpha[5] = convective_coef_natural_3(4,T_air_inside,T[5]);
