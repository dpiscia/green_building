...
data_fin = datetime(2010,6,29,10,05,0)
...
lista_fin = mf.lista_mod(lista_irr,lista_irr_free)
lista = list(set(lista_irr+ lista_night))
tran_weight = mf.transpiration_from_balance_irr(dati['peso_balanca'],300,2260000,lista) 

min_avg = 12 
  
tra_P_M_avg,time_P_M = df.avg(tra_P_M[lista_fin],lista_fin,min_avg)
tra_weigh_avg,time_weight = df.avg(tran_weight,lista_fin,min_avg)
