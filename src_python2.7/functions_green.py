...
def BC_cond(element,fluid_element,qcond,alphas,e,T,g,time):
    ...
    print "alpha", alphas[time][6]
    print "g[time][element]", g[time][element] 
    print "j[time][element]", e[6]*kbol*math.pow(T[time][6],4) + (1-e[6])*(g[time][6])
...
def BC_cond_2(element,fluid_element,qcond,qrad,alphas,T,time):
    ...
    /(alphas[time][element]))
    print "qcond", qcond
    print "qrad[time][element]", qrad[time][element]
    print "alphas[time][element] *T[time][fluid_element]", alphas[time][element] *(T[time][fluid_element]-T[time][element])
    print "qrad+qconv", qrad[time][element] + alphas[time][element] *(T[time][element]-T[time][fluid_element])
...
def BC_conv_conv_2(el_inner,el_outer,fluid_el_outer,fluid_el_inner,alphas,qrad,T,time):
    ...
    print "qrad0", qrad[time][0] 
    print "qrad4", qrad[time][4]    
    print  "qradouter+qconv_outer  ", qrad[time][el_outer] + alphas[time][el_outer] *(T[time][el_inner]-T[time][fluid_el_outer])
    print "qrad _inner +qconv_inner  ", qrad[time][el_inner] + alphas[time][el_inner] *(T[time][el_inner]-T[time][fluid_el_inner])
...
#               (geo.rho*geo.cp*Vol/deltaT)+(geo.cp*Ventilation));
       ...
       + (rho *cp * Vol * T[time-1][8]/ deltaT) ) / 
       ((rho *cp * Vol / deltaT) ) ) 
       print "sum convective", qconv[time][4]*A[4] + qconv[time][5]*A[5]+qconv[time][6]*A[6]
       print "gain thermal", (rho *cp * Vol * (T[time][8] - T[time-1][8])/ deltaT)
...
def relax(T,T0,time):
    T[time][0] = T[time][4]
    T[time][3] = T[time][5]
    print "Temp before relaxing", T[time]
    T_park = np.zeros(shape=(1,10))
    T_park = T[time]
    T[time][[0,4,3,5,6,8]] = T0[0][[0,4,3,5,6,8]] + 0.25*(T[time][[0,4,3,5,6,8]]-T0[0][[0,4,3,5,6,8]])
    print "temp after rel", T[time]
    print "T0", T0[0]
    print "tpark", T_park
    T0[0] = T[time]
    #raw_input("relaxing")
